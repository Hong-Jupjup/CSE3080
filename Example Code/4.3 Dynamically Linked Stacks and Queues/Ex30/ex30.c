...
int main()
{
    ...
    stacks[0] = (element*)malloc(sizeof(sizeof(element)*capacity[0]));
    stacks[1] = (element*)malloc(sizeof(sizeof(element)*capacity[1]));
...
{
    if(top[stackIdx] >= capacity[stackIdx] - 1)
        stackFull(stackIdx);
...
{
    count++;
    printf("number of stackf full called: %d\n", count);
    element *tmp = stacks[stackIdx];
    capacity[stackIdx]++;
    stacks[stackIdx] = (element*)realloc(stacks[stackIdx], sizeof(element) * capacity[stackIdx]);
    if(tmp != stacks[stackIdx]) {
