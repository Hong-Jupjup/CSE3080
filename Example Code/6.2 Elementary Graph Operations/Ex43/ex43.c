...
nodePointer graph[MAX_VERTCIES];
int parent[MAX_VERTCIES];
int dfn[MAX_VERTCIES];
int low[MAX_VERTCIES];
int num;
int n;
...
{
    if(dfn[u] == 0) {
        /* if u is root, then check wheter it has more than one child nodes. */
        int nChild = 0;
